...
// Basic test for M-mode installing hardware breakpoints which trigger in M-mode.

void __attribute__((naked)) mmode_nops() {
	asm volatile (
	".rept 4\n"
		"nop\n"
	".endr\n"
...
void set_mmode_breakpoint(int trigger, uintptr_t addr) {
...
int main() {
	...
	run_mmode_nops();
	tb_printf("mcause = %u\n", read_csr(mcause));
	tb_assert(read_csr(mcause) == 11, "should be M-mode ecall\n");
	...
	for (int t = 0; t < N_TRIGGERS; ++t) {
		...
		for (int n = 0; n < N_NOPS + 2; ++n) {
			...
			set_mmode_breakpoint(t, (uintptr_t)mmode_nops + n * nop_size);
			run_mmode_nops();
			unsigned int mcause = read_csr(mcause);
			uintptr_t offset = read_csr(mepc) - (uintptr_t)mmode_nops;
			tb_printf("  -> mcause = %u\n", read_csr(mcause));
			tb_printf("  -> mepc   = mmode_nops + %02x\n", offset);
			if (n == N_NOPS + 1) {
				tb_assert(mcause == 11, "should be ecall\n");
