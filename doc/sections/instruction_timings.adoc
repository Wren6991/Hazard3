...
| `mret` | 3 |
| `wfi` | 2+ | No upper limit.
3+| Memory Ordering
| `fence` | 1+ | Longer if preceded by load/store, or if `fence_rdy` is low.
| `fence.i` | 3+ | Longer if preceded by load/store, or if `fence_rdy` is low.
|===
...
|===
| Instruction | Cycles | Note
| `beqi rs1, imm, label`| 1 or 2footnote:unaligned_branch[] | 1 if correctly predicted, 2 if mispredicted.
| `bnei rs1, imm, label`| 1 or 2footnote:unaligned_branch[] | 1 if correctly predicted, 2 if mispredicted.
|===
