...
| 2 | Illegal instruction: fires when executing an instruction bit pattern that is not supported on this Hazard3 implementation, or when accessing a CSR that is unimplemented or inaccessible in the current privilege mode. Also fires when executing a `wfi` or `h3.block` in U-mode when `<<reg-mstatus>>.tw` = `1`.
...
One subtle detail is that, since the `h3.meinext.update` bit sets the `<<reg-h3.meicontext>>.preempt` level to the interrupt priority plus one, reading an interrupt from `h3.meinext` (with `update` set) and then re-enabling interrupts via `<<reg-mstatus>>.mie` will _never_ cause the core to take the same interrupt that was just read from `h3.meinext`.
...
| 23:21 | -           | RES0
| 20:16 | `preempt`   | Minimum interrupt priority to preempt the current interrupt. Interrupts with lower priority than `preempt` do not cause the core to transfer to an interrupt handler. Updated by hardware when when `h3.meinext.update` is written, or when hardware enters the external interrupt vector. Resets to `0`.
