...
NOTE: Versions of Hazard3 older than v1.1 allowed the implementer to set the `mimpid` contents arbitrarily, with a recommendation to set it to the git hash. The hashes of released versions are: `86fc4e3f` (RP2350, v1.0-rc1), `918aaee1` (v1.0), `82729101` (v1.0.1) and `787da131` (v1.0.2).
...
|===
| Bits | Name | Description
| 21    | `tw`  | Timeout wait. Only present if U-mode is supported. When 1, attempting to execute a WFI instruction in U-mode will instantly cause an illegal instruction exception. This also applies to the `<<h3.block-instruction>>` and `<<h3.unblock-instruction>>` instructions if the Xh3power extension is implemented.
...
| 3 | `msie` | Software interrupt enable. A software interrupt is requested when  `mie.msie`, `mip.msip` and `mstatus.mie` are all 1.
|===
...
NOTE: The RISC-V specification lists `mip` as a read-write register, but the bits which are writable belong to S-mode, which is not implemented on Hazard3.
...
|===
|Bits | Name | Description
| 11 | `meip` | External interrupt pending. When 1, indicates there is at least one interrupt which is asserted (hence pending in `<<reg-h3.meipa>>`), enabled in `<<reg-h3.meiea>>`, and of priority greater than or equal to `<<reg-h3.meicontext>>.preempt`.
...
Hardwired to 0, if <<param-U_MODE>> = `1`. Otherwise unimplemented.
...
|===
| Cause | Description
| 0 | Instruction address misaligned: fires when bit `1` of a branch target is set and <<param-EXTENSION_C>> is `0`. Note that bit `0` of a branch target is always implicitly masked in RISC-V, so this exception never fires when <<param-EXTENSION_C>> = `1`.
| 1 | Instruction access fault: fires when instruction fetch encounters an AHB error response. Instruction fetch faults are speculative, and a fault encountered in the prefetch of an instruction that is not subsequently executed is harmlessly flushed. Also fires on an attempt to execute an instruction which overlaps a PMP region that lacks execute permissions in the current privilege mode.
| 2 | Illegal instruction: fires when executing an instruction bit pattern that is not supported on this Hazard3 implementation, or when accessing a CSR that is unimplemented or inaccessible in the current privilege mode. Also fires when executing a `wfi`, `h3.block` or `h3.unblock` in U-mode when `<<reg-mstatus>>.tw` = `1`.
| 3 | Breakpoint: fires when executing an `ebreak` instruction, or as a result of a trigger (see <<section-trigger-module>>) that targets M-mode (i.e. `<<reg-tdata1>>.action` = `0`).
| 4 | Load address misaligned: fires when load address modulo load size is not equal to zero. Zilsd loads are treated as four bytes in size for alignment purposes, as they issue as pairs of 32-bit accesses.
| 5 | Load access fault: fires when a load instruction encounters an AHB error response. Also fires when loading from an address that lacks PMP read permissions in the current privilege mode.
| 6 | Store/AMO address misaligned: fires when store address modulo store size is not equal to zero. Zilsd stores are treated as four bytes in size for alignment purposes, as they issue as pairs of 32-bit accesses.
| 7 | Store/AMO access fault: fires when a store or AMO encounters an AHB error response. Also fires when storing to an address that lacks PMP write permissions in the current privilege mode, or an AMO attempts to access an address that lacks PMP read _or_ write permissions in the current privilege mode. Finally, an AMO will raise this exception if it issues an exclusive read that returns a negative `HEXOKAY` response (see <<section-exclusive-bus-transfers>>).
| 8 | Environment call from U-mode: fires when executing an `ecall` instruction if the current privilege mode is U-mode.
| 11 | Environment call from M-mode: fires when executing an `ecall` instruction if the current privilege mode is M-mode.
|===
...
Hardwired to 0. Hazard3 does not write trap information to the `mtval` register on any trap cause. Fault handlers and debuggers must reconstruct trap information by decoding the instruction at `<<reg-mepc>>` with reference to the trap cause in `<<reg-mcause>>`.

[NOTE]
====
The main use for `mtval` would be recording bus fault addresses. AHB returns faults in a later pipe stage than the addresses are issued, so reporting the address would require the address of all load/stores to be _speculatively_ registered into the next stage, even though the pipeline has no other use for it. This would have significant energy cost as well as area.

The other main use for `mtval` is recording the instruction bit pattern of illegal instructions; generally Hazard3 is deployed without I/D caches so the instruction can be re-fetched from `mepc` provided the address has both R and X PMP permissions, and the benefit of recording the instruction in `mtval` is marginal. Therefore Hazard3 elects to never record trap information.
====
...
This register only exists if <<param-U_MODE>> = `1`.
...
| 6:5 | -   | RES0
| 4:3 | `A` | Address-matching mode. Values supported depend on configuration. Value of 0 (OFF) is always supported. Value of 1 (TOR, top of range) is supported if <<param-PMP_MATCH_TOR>> = `1`. Values of `2` (NA4, naturally aligned 4-byte) and `3` (NAPOT, naturally aligned power-of-two) are supported if <<param-PMP_MATCH_NAPOT>> = `1`. Attempting to write an unsupported value will set the region to OFF.
...
Address registers for up to 16 physical memory protection regions. Only present if PMP support is configured. If so, all 16 registers are present, but some may be fully or partially hardwired.
...
* For TOR, the region matches all addresses greater than or equal to the next-lower-numbered `pmpaddr` register, and less than this `pmpaddr` register.
** As a special case, region 0 matches up from address `0`.
...
NOTE: To save energy in the default state, the `<<reg-mcountinhibit>>` `cy` bit resets to `1`. This bit must be cleared before `mcycle` starts to count.
...
NOTE: To save energy in the default state, the `<<reg-mcountinhibit>>` `ir` bit resets to `1`. This bit must be cleared before `minstret` starts to count.
...
An `action` of `0` is ignored when <<reg-tcontrol>>.`mte` is `0`. An `action` of `1` is ignored when `tdata1.dmode` is `0`.
...
| 9 | `m` | Write 1 to enable matching during M-mode execution, if <<reg-tcontrol>>.`mte` is also set. Self-clears when this trigger fires (in any privilege mode).
...
|===
| Bits | Name | Description
| 26 | `hit` | Hardwired to zero; this feature is not implemented
| 9 | `m` | Write 1 to enable matching during M-mode execution, if <<reg-tcontrol>>.`mte` is also set.
...
| 6 | `u` | Write 1 to enable matching during U-mode execution. Hardwired to zero if <<param-U_MODE>> = `0`.
| 5:0 | `action` | Hardwired to `1`; this trigger only supports breaking to Debug Mode (as an M-mode breakpoint exception triggered immediately after M-mode exception entry would be unrecoverable).
|===
...
Implements a two-level enable stack for triggers (<<section-trigger-module>>) which both trap into and originate from M-mode. Trapping into an M-mode trap handler disables M-mode triggers; the original enable state is restored on return from the handler. One example of where this is necessary is an M-mode trap handler single-stepping M-mode foreground code using the `icount` trigger: the trigger must remain disabled until after the core returns from the handler.

Since there are only two stack levels implemented in hardware, nested traps clear both bits. When using native M-mode triggers in combination with nested traps (for example, nested interrupts) you must save and restore `tcontrol` at each level of nesting.

[cols="10h,20,~", options="header"]
|===
| Bits | Name | Description
| 7 | `mpte` | M-mode previous trigger enable. When a trap into M-mode is taken, `mpte` is set to the value of `mte`.
| 3 | `mte` | M-mode trigger enable field. Setting `mte` = `0` inhibits triggers with <<reg-tdata1>>.`action` = `0` when the core is in M-mode.

When a trap into M-mode is taken, `mte` is set to `0`. When `mret` is executed, `mte` is restored from `mpte`.
|===
...
To provide data exchange between the Debug Module and the core, the Debug Module's `data0` register is mapped into the core's CSR space at a read/write M-custom address -- see `<<reg-h3.dmdata0>>`.
...
There are up to 512 external interrupts. When <<param-NUM_IRQS>> is less than 512, controls for IRQ numbers greater than or equal to <<param-NUM_IRQS>> are hardwired to zero. The upper half of this register contains a 16-bit window into the full 512-bit vector. The window is indexed by the 5 LSBs of the write data. For example:
...
A `1` bit indicates that interrupt is currently asserted. The value is the OR of each external IRQ signal with the corresponding force bit from `<<reg-h3.meifa>>`. IRQs are assumed to be level-sensitive, and in normal use the relevant `h3.meipa` bit is cleared by servicing the requester so that it deasserts its interrupt request. The interrupt force applied by `h3.meifa` clears automatically when the corresponding IRQ index is read from `<<reg-h3.meinext>>`, so the `h3.meipa` bit _also_ clears automatically if it was forced high by `h3.meifa`.

When any interrupt of sufficient priority is both set in `h3.meipa` and enabled in `h3.meiea`, the standard RISC-V external interrupt pending bit `mip.meip` is asserted. In other words, `h3.meipa` is filtered by `h3.meiea` (and preemption priority) to generate the standard `mip.meip` flag.

An external interrupt is taken when _all_ of the following are true:

* An interrupt is currently asserted in `<<reg-h3.meipa>>` (either due to the external interrupt signal being asserted high, or due to the corresponding bit in `<<reg-h3.meifa>>` being set).
* The matching interrupt enable bit is set in `<<reg-h3.meiea>>`.
* The interrupt priority is greater than or equal to the preemption priority, `<<reg-h3.meicontext>>.preempt`.
* The standard M-mode interrupt enable `<<reg-mstatus>>.mie` is set.
* The standard M-mode global external interrupt enable `<<reg-mie>>.meie` is set
...
The value `0x2c` here comes from the standard RISC-V vectoring logic for `<<reg-mtvec>>` with an IRQ cause of `meip`.
...
Bits can be cleared by software, and are cleared automatically by hardware upon a read of `h3.meinext` which returns the corresponding IRQ number in `h3.meinext.irq` (no matter whether `h3.meinext.update` is written). Each bit read from `h3.meipa` is the logical OR of the external interrupt signal and the corresponding `h3.meifa` bit.

The array resets to all-zeroes, meaning no interrupts are forced at reset.
...
External interrupt priority array. Each interrupt has an (up to) 4-bit priority value associated with it, and each access to this register reads and/or writes a 16-bit window containing four such priority values. When less than 16 priority levels are available, the LSBs of the priority fields are hardwired to 0. So for example, when <<param-IRQ_PRIORITY_BITS>> = `1`, the possible priority values are `0` and `8`.

Numerically higher values have higher priority. For example, an interrupt with priority `8` is taken before an interrupt with priority `0`. The interrupt priority array resets to all-zeroes, meaning all interrupts have the lowest priority by default.
...
| 15:7  | -        | RES0
| 6:0   | `index`  | Write-only, self-clearing field (no value is stored) used to control which window of the array appears in `window`. This is wider than the `index` fields on e.g. `<<reg-h3.meiea>>` because this register stores four times as many bits.
|===
...
Get next interrupt. Contains the index of the highest-priority external interrupt which meets all three of the following conditions:

* Is pending in `h3.meipa`.
* Is enabled in `h3.meiea`.
* Has priority greater than or equal to `<<reg-h3.meicontext>>.ppreempt`, the previous preemption level.

The returned interrupt index is left-shifted by 2 so that it can be used to index an array of 32-bit function pointers. If there is no such interrupt, the MSB is set, which can be checked efficiently using a `bltz` instruction.

When multiple interrupts of the same priority are both pending and enabled, the lowest-numbered wins. Interrupts with priority less than `h3.meicontext.ppreempt` -- the _previous_ preemption priority -- are treated as though they are not pending. This ensures that a preempting interrupt frame does not service interrupts which may be in progress in the frame that was preempted. More explicitly, an interrupt priority can fall into one of three possible ranges:

* Interrupt priorities _p_ in the range 0 ≤ _p_ < `ppreempt` are ineligible for processing at the current interrupt nesting level, and will not be processed until after the current interrupt stack frame is popped.
* Interrupt priorities _p_ in the range `ppreempt` ≤ _p_ < `preempt` are eligible for processing at the current interrupt nesting level. They appear in `h3.meinext` and do not cause the core to take an interrupt.
* Interrupt priorities _p_ in the range `preempt` ≤ _p_ ≤ 15 (max priority) are eligible for processing at a higher interrupt nesting level, and will cause the core to take an interrupt if interrupts are enabled. They also appear in `h3.meinext`.

Since the `h3.meinext.update` bit sets the `<<reg-h3.meicontext>>.preempt` level to the next level _above_ the sampled interrupt, reading an interrupt from `h3.meinext` (with `update` set) and then re-enabling interrupts via `<<reg-mstatus>>.mie` will never cause the core to take the same interrupt that was just read from `h3.meinext`. This avoids an interrupt service routine interleaving with itself, which would probably surprise the programmer.

A useful implication of this is that if the core takes some low-priority interrupt, and a higher-priority interrupt arrives while the interrupt dispatch routine is still saving registers to the stack, it is the _higher_-priority interrupt that will be processed first, and both interrupts are processed at the same level of nesting. This is sometimes referred to as _late arrival_.
...
`ppreempt` and `preempt` together define the range of interrupt priorities eligible for processing at the current level of interrupt nesting. Specifically this range is priorities _p_ in the range `ppreempt` ≤ _p_ < `preempt`. See `<<reg-h3.meinext>>` for more discussion on the relationship between preemption levels and interrupt nesting levels.

[cols="10h,20,~", options="header"]
|===
| Bits  | Name        | Description
| 31:28 | `pppreempt` | Previous `ppreempt`. Set to `ppreempt` on priority save, set to zero on priority restore.  Has no hardware effect, but ensures that when `h3.meicontext` is saved/restored correctly, `preempt` and `ppreempt` stack correctly through arbitrarily many preemption frames. Resets to `0`.
| 27:24 | `ppreempt`  | Previous `preempt`. Set to `preempt` on priority save, restored to `pppreempt` on priority restore. Resets to `0`.
...
| 23:21 | -           | RES0
| 20:16 | `preempt`   | Minimum interrupt priority to preempt the current context. Interrupts with lower priority than `preempt` do not cause the core to transfer to an interrupt handler. Updated by hardware when `h3.meinext.update` is written; when hardware enters the external interrupt vector (a _priority save_); or when executing an `mret` instruction with `h3.meicontext.mreteirq` = `1` (a _priority restore_). Resets to `0`.
...
This field is one bit wider than `ppreempt`. When the MSB of this field is set, its value is higher than the highest possible IRQ priority, meaning preemption is disabled. Such large values are never saved to `ppreempt`, because priority save happens only on preemption.
...
| 14:13  | -           | RES0
| 12:4   | `irq`       | Current IRQ number (read/write). Set to `h3.meinext.irq` when `h3.meinext.update` is written. Resets to `0`.
...
| 0      | `mreteirq` | Enable restore of the preemption priority stack on `mret`. This bit is set on entering the external interrupt vector, cleared by `mret`, and cleared upon taking any trap other than an external interrupt. Resets to `0`.
...
get_next_irq:
	...
	// h3.meinext, and write 1 to the LSB to tell hw to update h3.meicontext
	// with the preemption priority (and IRQ number) of this IRQ
...
| `0x500` | <<extension-xh3misa-section,Xh3misa>>
| `0x501` | <<extension-xh3irq-section,Xh3irq>>
| `0x502` | <<extension-xh3pmpm-section,Xh3pmpm>>
| `0x503` | <<extension-xh3power-section,Xh3power>>
| `0x504` | <<extension-xh3bextm-section,Xh3bextm>>
| `0x505` | <<extension-xh3sfx-section,Xh3sfx>>
|===
