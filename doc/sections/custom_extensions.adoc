...
Check two single-precision biased exponents for special cases. Returns `1` if either `rs1` or `rs2` is equal to either `0` or `255`. Otherwise returns `0`.

==== h3.fcheck2e.h

Check two half-precision biased exponents for special cases. Returns `1` if either `rs1` or `rs2` is equal to either `0` or `31`. Otherwise returns `0`.
