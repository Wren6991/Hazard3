...
Unpack significand from single-precision to Q3.29.
...
The exponent in `rs2` is a signed integer and must be in the range -512 ≤ _x_ ≤ 511.
...
The exponent in `rs2` is a signed integer and must be in the range -64 ≤ _x_ ≤ 63.
...
Shift right, arithmetic, with sticky bits collected at LSB. The shift is wrapped into the range 0 ≤ _x_ ≤ 255. Shifts ≥ 32 result in all-sign (plus the sticky bit).

NOTE: The expanded range of the shift amount avoids a special case branch for large exponent differences.
...
Shift signed-left, arithmetic. `rs2` is a signed integer wrapped to the range -256 ≤ _x_ ≤ 255. If `rs2` is positive, the result is `rs1 << rs2`. if `rs2` is negative, the result is `rs1 >> -rs2`. Left shifts ≥ 32 result in all-zeroes. Right shifts ≥ 32 result in all-sign.
...
__mulsf3:
    ...
    mul a1, a4, a5
    mulh a4, a4, a5
    // Normalise (shift left by either 3 or 4)
    h3.crsq3 a0, a4
    sll a4, a4, a0
    sub a2, a2, a0
    add a2, a2, a3
