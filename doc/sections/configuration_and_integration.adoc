...
The consequence for store data is a latency of two cycles (increased from one) no matter the source of the operand.
...
The value of `MULDIV_UNROLL` does not affect interrupt entry latency, because the core kills in-progress sequential multiply/divide operations when it needs to enter an interrupt handler or take an exception.
