...
These signals are used in the implementation of internal sleep states as configured by the `<<reg-h3.msleep>>` csr. They are used only when the Xh3power extension is enabled (<<extension-xh3power-section>>)

[options="header",cols="1,1,3,4"]
|===
| Width | In/Out | Name | Description
| 1          | Out | `pwrup_req`                  | Power-up request. Disconnect if Xh3power is not configured. Part of a four-phase (Gray code) req/ack handshake for negotiating power or clocks with your system power controller. The processor releases `pwrup_req` on entering a sufficiently deep `wfi` or `h3.block` state, as configured by the `<<reg-h3.msleep>>` CSR. It then waits for deassertion of `pwrup_ack` before taking further action.
...
| 1          | Out | `unblock_out`                | Pulses high when an `<<h3.unblock-instruction>>` instruction executes. Disconnect if Xh3power is not configured.
| 1          | In  | `unblock_in`                 | A high input pulse will release a blocked `<<h3.block-instruction>>` instruction, or cause the next `<<h3.block-instruction>>` instruction to immediately fall through. Tie low if Xh3power is not configured.

In a multiprocessor system, connect `unblock_in` to the OR combination of the `unblock_out` signals of all neighbouring cores. The exact definition of _neighbouring_ is the implementer's choice, but as a general rule: if two cores have shared access to any memory then their `unblock_out` and `unblock_in` signals should be connected.

In a single-core system, tie this input low.
|===
...
Once `fence_rdy` is seen high, the core flushes its prefetch buffer to order the fetch of younger instructions against the fence. It then resumes normal execution.
...
| 1          | Out | `dbg_running`                | Debug running status. Asserts when the processor is not halted and not transitioning between halted/running states. Disconnect if debug support is not configured.
...
The exact circuit used to implement these instructions, and the resulting area and performance, depends on the following parameters: <<param-MULDIV_UNROLL>>, <<param-MUL_FAST>>, <<param-MUL_FASTER>>, and <<param-MULH_FAST>>. See also the instruction timing listings in <<section-timings-m>>.
...
Custom power management controls for Hazard3. This adds the `<<reg-h3.msleep>>` CSR, and the `<<h3.block-instruction>>` and `<<h3.unblock-instruction>>` hint instructions. See <<extension-xh3power-section>>
...
NOTE: The following extensions are also recommended when Xh3sfx is enabled: <<param-EXTENSION_M>> = `1`, <<param-EXTENSION_XH3BEXTM>> = `1`, <<param-EXTENSION_ZBB>> = `1` and <<param-EXTENSION_ZBS>> = `1`.
...
the PMP is included (<<param-PMP_REGIONS>> is greater than `0`), the core may restrict U-mode software's access to
memory.
...
If _G_ > 0, `pmpcfg.a` cannot be set to NA4; attempting to do so will set the region to OFF instead. The _G_ LSBs of each `pmpaddr` register read back as all-zeroes when `pmpcfg.a` is set to TOR or OFF.
...
Remove all forwarding paths except X->X, to save area. This has a significant impact on per-clock performance, so should only be considered for extremely low-area implementations.

More specifically this eliminates the forwarding paths from stage 3 to stage 2 (M->X) and also the store data forwarding path within stage 3 (M->M). The consequences for ALU, multiplier and address operands are:

* Dependency on a load result has a latency of three cycles (increased from two).
* Dependency on an ALU or sequential multiply/divide result _not_ produced by the immediately prior instruction has a latency of three cycles (increased from one).
* Dependency on an ALU or sequential multiply/divide result produced by the immediately prior instruction has a latency of one cycle (no change).
* Dependency on a fast multiplier result when <<param-MUL_FASTER>> = `0`: same as a load result.
* Dependency on a fast multiplier result when <<param-MUL_FASTER>> = `1`: same as an ALU or sequential multiply/divide result.

Any result used for a store data operand (`rs2`) has a latency of three cycles (increased from one), no matter its source.
...
The sequential multiply/divide circuit has a base cost of 1 + 32 / `MULDIV_UNROLL` cycles, plus up to two sign correction cycles for `div`/`rem` and up to three sign correction cycles for `mulh`/`mulhsu`.

The value of `MULDIV_UNROLL` does not affect interrupt entry latency, because the core abandons in-progress sequential multiply/divide operations when it needs to enter an interrupt handler or take an exception.
...
Use a single-cycle multiply circuit for `mul` instructions. By default the fast multiplier produces a result in stage 3 (see <<param-MUL_FASTER>>), and the sequential multiply/divide circuit is still used for `mulh`, `mulhu` and `mulhsu` (see <<param-MULH_FAST>>).

Requires: <<param-EXTENSION_M>> = `1`.
...
Produce fast multiply results in stage 2 instead of stage 3.
The throughput is the same at 1/cycle, but latency is reduced from 2 cycles to 1 cycle.

This generally improves per-cycle performance, but reduces the portion of the clock period available for multiplier delay, which may affect overall timing.
...
Extend the fast multiply circuit to also cover `mulh`, `mulhu` and `mulhsu`. When this configuration is enabled, these instructions execute with the same timings as `mul`. This increases both the area and delay of the fast multiplier. The unnecessary multiply functionality is removed from the sequential multiply/divide circuit.
...
This parameter only controls the register file reset. The majority of flops in Hazard3 are always reset, regardless of whether this parameter is set. The only exception besides the register file is the datapath flops in the fast multiplier (<<param-MUL_FAST>> = `1` and <<param-MUL_FASTER>> = `0`), which are never reset, to improve FPGA DSP inference compatibility.
...
Successful prediction eliminates the 1-cycle fetch bubble on a taken branch, so tight loops execute faster. See <<instruction-timings-branch-predictor>> for more information on this feature.
