...
This parameter only controls the register file reset. The majority of flops in Hazard3 are always reset, regardless whether this parameter is set. The only exception besides the register file is the datapath flops in the fast multiplier (<<param-MUL_FAST>> = `1` and <<param-MUL_FASTER>> = `0`), which are never reset, to improve FPGA DSP inference compatibility.
